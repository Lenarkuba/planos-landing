  ...
  return (
    <section id="security" className="py-16 md:py-24 px-4 md:px-8 max-w-[1200px] mx-auto">
      <AnimateIn>
        ...
        >
          PESEL-e <em className="italic text-[var(--accent)]">chronione dzięki technologii bankowej</em>
        </h2>
      ...
      <AnimateInStagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
        {securityItems.map((item) => (
          <AnimateInChild key={item.title} className="h-full">
          <div className="h-full rounded-2xl bg-[var(--bg-card)] p-8 transition-colors hover:bg-[var(--bg-card-hover)] flex flex-col">
